...
let e_a_screen = [document.getElementById("id_screen_a"), document.getElementById("id_screen_b"), document.getElementById("id_screen_c")];
e_a_screen[0].classList.add("cl_screenPage");
e_a_screen[1].classList.add("cl_screenPage");
e_a_screen[2].classList.add("cl_screenPage");

let e_debugMask = document.getElementById("id_debugMask");
let showDebugMask = false;
if (!showDebugMask) e_debugMask.classList.add("cl_displayNone");

let e_crtScreenMask = document.getElementById("id_crtScreenMask");
...
let a_screenSpace = [];
let a_igniterDrops = [];
let a_extinguisherDrops = [];

let e_inputTextFrameLength = document.getElementById("id_input_text_frame_length");

let screenSpaceX; // = 60;
let screenSpaceY; // = 30;
const screenSpaceX_max = 60;
const screenSpaceY_max = 30;
...
let igniterDrop_waitForTwin = false;
let extinguisherDrop_waitForTwin = false;
let changeCharactersNthFrames = 3;

let timeStart = 0;
let timeFrameLength = 100;
e_inputTextFrameLength.value = timeFrameLength;

let frameCounter = 0;
let stopAtFrame_debug = -1;
let visiblePage = 0;
let restingPage = 1;
let drawingPage = 2;
...
function initScreenSpace() {
    for (let j = 0; j < screenSpaceY; j++) {
        ...
        for (let i = 0; i < screenSpaceX; i++) {
            ...
            let t_state = 0;
            let t_a_element = [document.createElement("span"), document.createElement("span"), document.createElement("span")];
    ...
    for (let j = -maxBlindColumnHeight; j < screenSpaceY; j++) {
        for (let i = 0; i < screenSpaceX; i++) {
            if (Math.random() < 0.03) {
                ...
                //conLog("columnLength: " + columnLength);
...
function step() {
    ...
    flipPages();
    frameCounter++;
//    conLog("frameCounter: " + frameCounter);
//    conLog("drawingPage: " + drawingPage);
...
function determineScreenSpace() {
    let x = Math.floor(window.innerWidth / 11)-12;
    let y = Math.floor(window.innerHeight / 24 * 1.2);

    if (x > screenSpaceX_max)
        x = screenSpaceX_max;
    if (y > screenSpaceY_max)
        y = screenSpaceY_max;
...
function clearEnvironment() {
    a_screenSpace.length = 0;
    a_igniterDrops.length = 0;
    a_extinguisherDrops.length = 0;
    ...
    [screenSpaceX, screenSpaceY] = determineScreenSpace();

    e_a_screen[0].innerHTML = "";
    e_a_screen[1].innerHTML = "";
    e_a_screen[2].innerHTML = "";
    e_debugMask.innerHTML = "";
